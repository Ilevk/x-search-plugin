---
name: x-search-plugin
description: Use when the user wants read-only X/Twitter research, recent posts, public reactions, trends, account-specific posts, or X-sourced evidence through the local x-search-plugin MCP tools.
---

# X Search Plugin

Use this skill for read-only X/Twitter research through the local
`x-search-plugin` MCP server.
...
The login stores token state under `~/.x-search-plugin/auth.json` and the MCP
server refreshes the access token before X Search calls.
Legacy `~/.x-search-codex/auth.json` credentials remain usable when the new
...
Tell them to run `uv run --quiet --locked python scripts/x_search_auth.py login` locally, or put a
...
- Missing credential: ask the user to run `uv run --quiet --locked python scripts/x_search_auth.py login`
